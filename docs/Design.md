...
The answer is remembered in `.penv/targets/<name>/target.toml` — `name`, `output`, and an `[options]` table holding every knob at the value in effect, each under a `# penv:` line saying what it changes and what it takes — so a repository is asked once, ever, and the knobs are in the file it already commits rather than in a manual. That holds for an explicit flag too, and for an answer that happens to equal the built-in default, because it was still an answer. A folder that says more than that, or that carries a `#` line penv did not write, was written by hand and is never rewritten; a value edited in place is read back and written back. `.penv/targets` is committed. Only the generated file moves: `.env.schema` and the gitignore stay at the root. A path outside the repository is refused rather than written, whether it arrived as an absolute path or as a `..`, and `.` and `..` are taken out of a path before anything downstream sees it.
