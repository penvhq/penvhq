...
**Implementation debt (penv-cloud).** What the CLI and `@penvhq/varlock-plugin` read, and the server does not yet guarantee. The first four are missing; the rest must be verified against the deployed API before the plugin's first release is announced:
...
4. **The OIDC audience** (found in review). The server's `authenticateOidc` requires `aud` to be the org's id, a UUID, and refuses anything else as `invalid`; the Connect stub mints for the id too. On GitHub Actions the CLI mints the token itself, with the org's slug from `@penv=` as the audience, so every exchange the CLI mints is refused. Until one side moves, a job mints its own token for the org id and hands it over in `PENV_OIDC_TOKEN`. The fix is one of: the server resolves a slug `aud` to the org before the trust lookup, still checking the signature against the audience it stored; or the CLI reads the org id from `PENV_OIDC_AUDIENCE`, which the console shows beside the trust.
...
8. **No redirects on the API.** Both clients refuse a redirect instead of following it, so `/api/v1/*` must answer directly, with no trailing-slash or locale redirect in front of it.
